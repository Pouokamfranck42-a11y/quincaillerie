<x-layout title="Caisse">
    ...
    <div class="page-head" x-data="{ closing: false }">
        <div>
            <h1><i class="bi bi-cash-coin text-primary"></i> Caisse</h1>
            <p>Session ouverte le {{ $session->opened_at->format('d/m/Y à H:i') }} · fond de caisse {{ number_format($session->opening_amount, 0, ',', ' ') }} FCFA</p>
        </div>
        <div>
            <button type="button" class="btn" @click="closing = true" x-show="!closing"><i class="bi bi-door-closed"></i> Fermer la caisse</button>
            <form method="POST" action="{{ route('cash-sessions.close') }}" class="flex" x-show="closing" x-cloak>
                ...
                <button type="submit" class="btn btn-primary btn-sm"><i class="bi bi-check-lg"></i> Confirmer</button>
